...
class DoclensController extends ChangeNotifier {
  ...
  ///
  /// Seeded from [ScannerConfig.enableAutoCapture] and settable at runtime,
  /// so a manual/auto toggle does not have to dispose the controller and
  /// restart the native session. Disabling resets the stability and focus
  /// state and abandons any confirmation window in flight, so re-enabling
  /// starts from a clean alignment rather than resuming a stale one.
